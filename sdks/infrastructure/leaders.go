...
// CreateLeaders creates a new Leaders SDK instance with a pointerID
...
func (sdklead *leaders) SaveTrs(serv servers.Server, trs aggregated.Transactions) (aggregated.SignedTransactions, error) {
	...
	if userSigErr != nil {
		str := fmt.Sprintf("there was an error while building the pointer signature: %s", userSigErr.Error())
		return nil, errors.New(str)
	}
