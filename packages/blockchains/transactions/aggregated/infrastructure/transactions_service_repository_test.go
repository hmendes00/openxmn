...
func TestSaveTrs_thenRetrieve_Success(t *testing.T) {
	...
	//execute:
	repository := CreateTransactionsRepository(metaDataRepository, signedTransRepository, atomicTransRepository, aggregatedTrsBuilderFactory)
	service := CreateTransactionsService(metaDataService, signedTransService, atomicTransService, aggregatedStoredTrsBuilderFactory)
