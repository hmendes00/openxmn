...
import (
	...
	concrete_validated_blocks "github.com/XMNBlockchain/core/packages/blockchains/blocks/validated/infrastructure"
	...
	concrete_stored_chained_blocks "github.com/XMNBlockchain/core/packages/storages/blocks/chained/infrastructure"
...
func TestSaveChainedBlk_thenRetrieve_Success(t *testing.T) {
	...
	//create the block:
	chainedBlk := CreateBlockForTests(t)
	...
	validatedBlkBuilderFactory := concrete_validated_blocks.CreateBlockBuilderFactory(htBuilderFactory, metaDataBuilderFactory)
	storedValidatedBlkBuilderFactory := concrete_stored_validated_blocks.CreateBlockBuilderFactory()
	validatedBlkRepository := concrete_validated_blocks.CreateBlockRepository(metaDataRepository, signedBlkRepository, userSigsRepository, validatedBlkBuilderFactory)
	validatedBlkService := concrete_validated_blocks.CreateBlockService(metaDataService, signedBlkService, userSigsService, storedValidatedBlkBuilderFactory)
	chainedBlkMetaDataBuilderFactory := CreateMetaDataBuilderFactory()
	chainedBlkBuilderFactory := CreateBlockBuilderFactory(htBuilderFactory, chainedBlkMetaDataBuilderFactory)
	storedChainedBlkBuilderFactory := concrete_stored_chained_blocks.CreateBlockBuilderFactory()
	chainedBlkMetaDataRepository := CreateMetaDataRepository(fileRepository)
	chainedBlkMetaDataService := CreateMetaDataService(fileBuilderFactory, fileService, storedFileBuilderFactory)
	...
	//execute:
	repository := CreateBlockRepository(chainedBlkMetaDataRepository, validatedBlkRepository, chainedBlkBuilderFactory)
	service := CreateBlockService(chainedBlkMetaDataService, validatedBlkService, storedChainedBlkBuilderFactory)
