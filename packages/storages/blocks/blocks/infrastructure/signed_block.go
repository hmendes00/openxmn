...
func createSignedBlock(metaData stored_files.File, sig stored_files.File, blk stored_blocks.Block) stored_blocks.SignedBlock {
	out := signedBlock{
		metaData: metaData,
		sig:      sig,
		blk:      blk,
...
func (blk *signedBlock) GetSignature() stored_files.File {
	return blk.sig
}
