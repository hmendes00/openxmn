...
type Block struct {
	...
	Blk *concrete_blocks.Block      `json:"block"`
	LS  []*concrete_users.Signature `json:"leader_signatures"`
}

func createBlock(ht *concrete_hashtree.HashTree, blk *concrete_blocks.Block, ls []*concrete_users.Signature) validated.Block {
	out := Block{
		HT:  ht,
		Blk: blk,
		LS:  ls,
...
func (blk *Block) GetBlock() blocks.Block {
	return blk.Blk
}
